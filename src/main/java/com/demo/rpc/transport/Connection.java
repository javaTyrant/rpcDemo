...
public class Connection implements Closeable {
    ...
    // TODO 时间轮定时删除
    public final static Map<Long, NettyResponseFuture<Response, Request>> IN_FLIGHT_REQUEST_MAP
    ...
    public NettyResponseFuture<Response, Request> request(Message<Request> message, long timeOut) {
        // 生成并设置消息ID
        long messageId = ID_GENERATOR.incrementAndGet();
        message.getHeader().setMessageId(messageId);
        // 创建消息关联的Future
        NettyResponseFuture<Response, Request> responseFuture = new NettyResponseFuture<>(System.currentTimeMillis(),
    ...
    public boolean ping() {
        ...
        NettyResponseFuture<Response, Request> request = request(message, Constants.DEFAULT_TIMEOUT);
        try {
